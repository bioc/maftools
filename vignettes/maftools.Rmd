...
```{r,fig.height=3,fig.width=7,fig.align='left'}
...
[MutationMapper](http://www.cbioportal.org/mutation_mapper.jsp) on cBioPortal collapses Variant Classifications into truncating and others. It also includes somatic mutation rate.

```{r, warning=FALSE, message=FALSE, fig.height=3,fig.width=7,fig.align='left'}
...
```{r, fig.align='left', fig.height=4.5, fig.width=8, message=FALSE, results='hide'}
...
vafPlot = plotVaf(maf = laml, vafCol = 'i_TumorVAF_WU', flip = FALSE)
```
...
```{r, fig.align='default', fig.width=5, message=F,results='hide', eval=T}
laml.sig = oncodrive(maf = laml, AACol = 'Protein_Change', minMut = 5, pvalMethod = 'zscore')
head(laml.sig)
```

We can plot the results using `plotOncodrive`.
```{r, fig.align='left', fig.width=5, fig.height=4, eval= T}
...
```{r, echo = TRUE, fig.align='left', fig.width=5, eval=T}
...
```{r, fig.align='left',fig.width=5, eval=T}
...
```{r, eval=TRUE, fig.width=6, fig.height=5, fig.align='left'}
plotApobecDiff(tnm = laml.tnm, maf = laml, title_size = 0.9)
```
...
```{r, fig.width=5, fig.align='left', eval = T}
plotSignatures(laml.sign, title_size = 0.7)
```
...
```{r, fig.width=7, fig.height=2.5, fig.align='left'}
require('pheatmap')
